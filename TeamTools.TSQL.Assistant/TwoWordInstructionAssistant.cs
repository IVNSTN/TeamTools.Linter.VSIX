...
{
    public sealed class TwoWordInstructionAssistant : BaseCodeEditingAssistant
    {
        public TwoWordInstructionAssistant(ITextOutputPort outputPort, string code) : base(outputPort, code)
        {
        }
        ...
        public override bool Parse()
        {
            ...
            var codeModifier = new TwoWordInstructionViolationDetector();
            sql.Accept(codeModifier);

            HasChanges = codeModifier.TwoWordInstructionsToFix.Count > 0;
            if (HasChanges)
            {
                FixTwoWordInstructions(codeModifier.TwoWordInstructionsToFix);
            }
        ...
        // TODO : extract from concrete assistants
        private void PerformModifications(IOrderedEnumerable<TwoWordInstructionCodeFix> blocks, IList<string> code)
        {
        ...
        // TODO : extract from concrete assistants
        private void FixTwoWordInstructions(IList<TwoWordInstructionCodeFix> blocks)
        {
            var modifiedCodeLines = new List<string>(OriginalCode.Split(new string[] { Environment.NewLine }, StringSplitOptions.None));
            var reversePositionList = blocks.OrderByDescending(b => b.StartLine).ThenByDescending(b => b.StartCol);
